...
public class EtherTCPAddRemoveModule implements IReorderModule, ILoggable
{
    public final static String REORDER_NAME = "EtherTCPAddRemove";
        ...
        FloodlightReorder _floodlight_reorder)
    {
        ...
        {            
            // flow mod add,
            flowmod_list.add(
                protocol_util.generate_add_eth_src_and_tcp_src_port_flow_mod(
                    i,i));


            // every other time, remove based on tcp source and other
            // times based on ethernet source.
            OFFlowMod rm_flow_mod = null;
            if ((i % 2) == 0)
            {
                rm_flow_mod =
                    protocol_util.generate_rm_eth_src_flow_mod(i);
            }
            else
            {
                rm_flow_mod =
                    protocol_util.generate_rm_tcp_src_port_flow_mod(i);
            }
            
            // flow mod remove
            flowmod_list.add(rm_flow_mod);
        }
    }
    ...
    public enum Factory implements IReorderModuleFactory
    {
        ...
        {
            return new EtherTCPAddRemoveModule();
        }
    }
}
